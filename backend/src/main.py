...
# print(os.getcwd())
# print(f for f in listdir('/dist') if isfile(join('/dist', f)))

# Enable CORS for all domains on all routes
# CORS(app, resources={r"/*": {"origins": "http://localhost:4173"}})
# CORS(app, resources={r"/*": {"origins": "http://localhost:5174"}})
CORS(app, resources={r"/*": {"origins": "http://192.168.0.64:4173"}})
...
# --------------- USER DATA --------------- #
# Fetch from table -- User Demographics
# USER_DATA = {
#         "USER_NAME": "Joanna",
#         "USER_DOB": datetime.date(year=2002,month=10,day=18).strftime('%Y-%m-%d'),
#         "USER_AGE": 20,
#         "USER_SEX": "F",
#         "SESSION_DATE": datetime.date(year=2024,month=6,day=27).strftime('%Y-%m-%d'),
#         "TIME_SINCE": round((TODAY - datetime.date(year=2023,month=10,day=18)).days/30.44, 0),
...
# --------------- HEADERS + LABELS --------------- # For better translation purposes
...
# --------------- USER METRICS --------------- # Gameplay data from the user
# Reading in tables - Primary Metrics
...
# print(USER_METRICS['EFCS']['symbol'], USER_METRICS['TACR']['symbol'], USER_METRICS['MDS']['symbol'], USER_METRICS['SAR']['symbol'], USER_METRICS['SAV']['symbol'], USER_METRICS['SAD']['symbol'])

# Variables for inserting into auto-gen text: -- eval(f'f"""{}"""')
...
# ----------- ABOUT METRICS TAB ----------- # For the about metrics page -- put into markdown files to ease translations
md_paths = {
